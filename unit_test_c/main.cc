...
int main(int argc, char **argv) {
  ...
  DirectionCosine d_cos =
      DirectionCosine(1.0 / sqrt(3.0), 1.0 / sqrt(3.0), 1.0 / sqrt(3.0));
  ...
  bool belong_x =
      BELONGS(intersection_pt, facet_coords0, facet_coords1, facet_coords2, x);
  bool belong_y =
      BELONGS(intersection_pt, facet_coords0, facet_coords1, facet_coords2, y);
  bool belong_z =
      BELONGS(intersection_pt, facet_coords0, facet_coords1, facet_coords2, z);
  printf("belong_x: %u\n", belong_x);
  printf("belong_y: %u\n", belong_y);
  printf("belong_z: %u\n", belong_z);
  double cross1 =
      AB_CROSS_AC(facet_coords0.x, facet_coords0.y, facet_coords1.x,
                  facet_coords1.y, intersection_pt.x, intersection_pt.y);
  double cross2 =
      AB_CROSS_AC(facet_coords1.x, facet_coords1.y, facet_coords2.x,
                  facet_coords2.y, intersection_pt.x, intersection_pt.y);
  double cross0 =
      AB_CROSS_AC(facet_coords2.x, facet_coords2.y, facet_coords0.x,
                  facet_coords0.y, intersection_pt.x, intersection_pt.y);
  ...
  printf("###########################\n");
  double e = 15.032;
  double speed = Get_Speed_From_Energy(e);
  printf("speed: %f\n", speed);
}